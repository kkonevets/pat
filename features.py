...
def distribured(w2v_model, samples, all_ids, corpus_files, fname, n_chunks=50):
    wv = w2v_model.wv
    ...
    ftrs = []
    for samp_part in tqdm(chunkify(samples, n_chunks)):
        res = Parallel(n_jobs=cpu_count, backend="multiprocessing") \
            (delayed(distributed_worker)(all_ids, docs_in_ram, index2word, wv, part) for
